...
class BookAdapter(private val onSelectionChanged: (BookCandidate, Boolean) -> Unit) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val row = rows[position]) {
            is Row.Book -> {
                ...
                holder.itemView.setOnClickListener { toggleBook(row.book) }
            }
            is Row.Series -> {
                val expanded = row.group.key in expandedGroups
                val selectedCount = row.group.books.count { it.bookId in selectedIds }
                (holder as SeriesHolder).bind(row.group, expanded, selectedCount)
    ...
    class SeriesHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(group: LibraryGroup, expanded: Boolean, selectedCount: Int) {
            ...
            selection.visibility = if (selectedCount == 0) View.GONE else View.VISIBLE
    ...
    class BookHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: BookCandidate, selected: Boolean, inSeries: Boolean) {
            ...
            val formatDetails = if (item.isComic && item.displayedComicPages != null) {
                "${item.displayedComicPages} pages"
